...
function uniqueLinear(words) {
  ...
  for (
    ...
    i < words.length;               // 2 * n = O(n) linear
    i++                             // 2 * n = O(n) linear
  ) {
    const word = words[i];          // 2 * n = O(n) linear
    if (!seen[word]) {              // 3 * n = O(n) linear
      seen[word] = true;            // 2 * n = O(n) linear
      unique[unique.length] = word; // 3 * n = O(n) linear
    }
  }
...
function uniqueQuadratic(words) {
  ...
  for (
    ...
    for (
      ...
      c < i;                        // 1 * n^2 = O(n^2) quadratic
      c++                           // 2 * n^2 = O(n^2) quadratic
    ) {
      const comparing = words[c];   // 2 * n^2 = O(n^2) quadratic
      if (comparing === word) {     // 2 * n^2 = O(n^2) quadratic
